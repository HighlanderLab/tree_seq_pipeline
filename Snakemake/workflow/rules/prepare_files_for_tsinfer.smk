...
rule get_af:
    input: f'../{Project}/VCF/{{chromosome}}_phased.vcf.gz'
    output:
        info = temp(f'../{Project}/VCF/Info{{chromosome}}.INFO')
    params:
        prefix=f'../{Project}/VCF/Info{{chromosome}}'
    conda: "bcftools"
    threads: 1
...
rule get_major:
    ...
    output: temp(f'../{Project}/VCF/Major{{chromosome}}.txt')
    threads: 1
    resources: cpus=1, mem_mb=32000, time_min=60
...
rule extract_ancestral_chromosome:
    input:
        ancestral_file
    output:
        temp(f'../{Project}/VCF/Ancestral{{chromosome}}.txt')
    params:
...
rule join_major_ancestral:
    ...
    output:
        temp(f'../{Project}/VCF/MajAAnc_{{chromosome}}.txt')
...
rule determine_ancestral_major:
    ...
    output:
        temp(f'../{Project}/VCF/MajOAnc_{{chromosome}}.txt')
...
rule decompress:
    ...
    output: f'../{Project}/VCF/{{chromosome}}_phased.vcf' # this is removing both the .gz and the decompressed file
    threads: 1
    resources: cpus=1, mem_mb=32000, time_min=30
...
rule change_infoAA_vcf:
    ...
    output: f'../{Project}/VCF/{{chromosome}}_ancestral.vcf'
    conda: "bcftools"
    threads: 1
...
rule compress_vcf:
    ...
    output:
        file = f'../{Project}/VCF/{{chromosome}}_ancestral.vcf.gz',
        idx = f'../{Project}/VCF/{{chromosome}}_ancestral.vcf.gz.csi'
    conda: "bcftools"
    threads: 1
