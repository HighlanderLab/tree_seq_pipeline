...
if len(splitFiles) > 0:
    rule move_vcf:
        input:
            expand(f'{vcfdir}/RawVCF/{{chromosome}}_{{suffixOne}}.vcf.gz', chromosome = chromosomes, suffixOne = splitFiles)
        output:
            expand(f'{vcfdir}/{{chromosome}}/{{chromosome}}_{{suffixOne}}.vcf.gz', chromosome = chromosomes, suffixOne = splitFiles)
        shell:
            """
            mv {input} {output}
...
if len(combinedFiles) > 0:
    rule split_and_move_vcfs:
        input:
            expand(f'{vcfdir}/RawVCF/Combined_{{suffixTwo}}.vcf.gz', suffixTwo = combinedFiles)
        output:
